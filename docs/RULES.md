...
- **Default run** (`nabokov file`) enables the core checks; run `nabokov --list-rules` for the current set.
...
| `NB317` | hidden-character | blue | An invisible or look-alike character: zero-width space, zero-width (non-)joiner, word joiner, a BOM mid-text, or a Cyrillic/Greek letter sandwiched inside a Latin word (`Naus`**`й`**`e` for *Nausée*). Breaks search, copy-paste, diffs, screen readers, and any code that contains it. Invisible characters are a `replace` (deleting one changes nothing around it); a homoglyph is a `rewrite`, since the tool cannot know which alphabet was meant. NBSP and narrow NBSP are deliberately absent — Russian and French typography use them. The sandwich shape keeps ordinary multilingual text clean: whole-script words, a Cyrillic suffix on a Latin brand ("в Slackе"), and unit prefixes ("μs") have no interior mixed letter. Scans the **original** text, so it still fires inside a code fence — the case that does the most damage. **Split out of NB519 and turned on by default**: the detection research shows a detector normalizes these away before scoring, so their presence says nothing about authorship. It is a defect for every reader, whoever wrote the text. | `The plan`**`\u200b`**` works` |
