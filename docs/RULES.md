...
|------|------|-----|-------|---------|
| `NB501` | ai-negation-contrast | warning / info | "it's not X, it's Y" / "not only X but Y" antithesis, and "No X, no Y, just Z". Also (info) two split-sentence reframes: the "no longer" reframe — the same third-person subject renamed across two sentences with a copula swap — and the "doesn't mean" reframe — a negated definition completed by an "it means Y" reveal (both halves required; the bare negation is ordinary prose and exempt, e.g. PG's "Writing essays doesn't have to mean publishing them."). First person ("I'm no longer at the bank. I'm…") is autobiography and exempt. Also (info) the "more than just" reframe — a copula + "more than just" ("This **is more than just** a tool"), the positive twin of "isn't just X"; a plain verb ("we offer more than just speed") is exempt. Also (info) the negation → role-reveal couplet: a negated obligation completed by a possessive role sentence — "A skill **does not need to** teach it. **Its job is** activation." Both halves required (zero occurrences across 361k words of Emerson, Chesterton, Dickens, Thoreau); the bare negation or the bare role sentence is ordinary prose and exempt. Also (info) the appearance-verdict couplet: a short "how it looks" sentence ("This feels/seems/looks/sounds X.") followed by a short declarative verdict. A question follow-up is inquiry, not verdict, and exempt ("This seems perfect. But is it?" — V. Nabokov); so are spoken replies (comma in the appearance half) and the inspection sense "looks at". | "This **isn't just fast, it's** transformative."; "**They're no longer** entertainment. **They're** market research."; "Market research **doesn't have to mean** spreadsheets. **Sometimes it means** listening."; "**This feels pointless. It is not.**" |
...
| `NB506` | ai-em-dash | warning | Em-dash overuse — a *density* above the human range (> 12 per 1000 words; essayists reach ~11). Counts `—`, spaced `–`, and the spaced ASCII hyphen ("word - word"); list bullets don't count. | "It was fast — clean — simple — done." |
...
| `NB509` | ai-monotonous-rhythm | info / warning | Flat sentence rhythm (low burstiness — the sentence-length CV). The threshold is per target (short-form registers like `SOCIAL` tolerate flatter rhythm); a genuinely robotic rhythm escalates to a warning. Calibrated so the essayist corpus (lowest CV 0.44) never fires. Read the CV for any file with `--stats`. | uniform mid-length sentences throughout |
...
| `NB518` | ai-adjective-triad | info | The symmetry attractor: every enumeration pressed into a balanced adjective triple. A *density* tell: the tricolon is 2,000 years of rhetoric. Essayists measure under 0.5 triads per 1000 words, so only 1.5+/1000 (min 2) shows the reflex. Exception: a triad launched by a copula-colon ("…the signal **is:** X, Y, and Z") fires alone — style guides bar a colon straight after "is" (zero corpus occurrences), while LLM broetry leans on the shape; the human colon-list rides behind a noun ("three things: …") and is not flagged. | "**innovative, transformative, and groundbreaking**"; "the real signal is: **spontaneous, unfiltered, and impossible** to fake" |
...
| `NB522` | ai-engagement-bait | info | Engagement-bait closer: the document signs off with a broad second-person superlative question — the algorithmic-reach CTA that ends LLM-drafted social posts. Flags *bait*, not AI: humans growth-hack with the same shape, so it stays advisory. Needs a superlative (or "your favorite"), a "you" + verb clause, and the question ending the final paragraph — "What's the best way to reach you?" has no verb after "you" and stays clean. | "**What's the most unexpected place you've found genuine customer insight?**" |
| `NB523` | ai-anaphora-triad | info | In-sentence anaphora triad: the same quantifier (more/less/fewer/every/each) opening three coordinated phrases — the symmetry attractor applied to a comparative (NB507/NB518's sibling). Formally identical to legitimate rhetoric — Emerson, Dickens, and Thoreau all use the shape (8 hits across 361k words, ~0.02/1000) — so advisory: only intent separates them. A pair, or a varied enumeration, is exempt. "No X, no Y, just Z" belongs to `NB501`. | "**more code reviews, more reports, and more style guides**" |
| `NB524` | ai-contrast-heading | info | The "X, not Y" contrast heading — `NB501`'s negation-contrast compressed into a title. The thinnest tell in the catalogue: it fires on legitimate human titles ("Ask forgiveness, not permission"), and no heading corpus backs it, so treat a hit as "look at this heading", never as evidence by itself. A grounded contrast stays; a document whose every heading corrects something is the actual signal. Headings only; the same shape in running text is `NB501`'s territory. | "## **Pin decisions, not knowledge**" |
| `NB525` | ai-hook-question | info | The hook-question fragment: a verbless 2–4-word noun-phrase question answered by the next sentence — the self-interview cadence. Real questions ("Why? Because…"), fragments with a verb ("Sound familiar?"), and a question that opens a new paragraph are exempt. Humans use the shape as rhetoric, so advisory. | "**The best part? It's free.**" |
| `NB526` | ai-false-range | info | The false range: "from X to Y" where X and Y aren't points on any scale. Grounded in the concreteness norms — needs an abstract endpoint (min < 2.2) and an abstract pair (mean < 3.0), so "from London to Paris" (proper nouns), "from 9 to 5" (numbers), "from start to finish" (concrete pair), and transfers behind a motion verb ("moved from marketing to consulting") are exempt. Advisory: some abstract pairs are honest progressions. | "everything **from strategy to execution**" |
| `NB527` | ai-uniform-paragraphs | info | Uniform paragraph length — every paragraph the same number of sentences (NB509's paragraph-level sibling). Measures the CV of sentences-per-paragraph; human docs measure 0.77+, a steady 3-4-sentences-each draft measures near 0. Needs ≥ 6 prose paragraphs and a mean ≥ 2 sentences; all-one-sentence documents are line-oriented style, not the tell. Document-level. | eight paragraphs, three sentences each |
