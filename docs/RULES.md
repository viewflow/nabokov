...
| `NB524` | ai-contrast-heading | info / warning | The "X, not Y" contrast heading — `NB501`'s negation-contrast compressed into a title. A single hit is advisory: it fires on legitimate human titles ("Ask forgiveness, not permission"), so treat it as "look at this heading". Two or more such headings in one document — every heading correcting something — is the actual signal and escalates to warning. Headings only; the same shape in running text is `NB501`'s territory. | "## **Pin decisions, not knowledge**" |
